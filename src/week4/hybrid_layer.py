# src/week4/hybrid_layer.py
# Robust helpers for LoRA / NormTune on timm ViT blocks, plus method builder.
...
# ---------------------------
# LoRA: minimal Linear adapter
# ---------------------------
class LoRALinear(nn.Module):
    """
    Wraps a frozen Linear weight/bias and adds trainable low-rank adapters A,B.
    Forward: x @ W^T + scale * (x @ A @ B) + bias
    """
    def __init__(
        self,
        base: nn.Linear,
...
def _replace_linear_with_lora(module: nn.Module,
                              target_modules: Optional[List[str]] = None,
                              r: int = 8,
                              lora_alpha: float = 16.0,
                              lora_dropout: float = 0.0):
    """Recursively wrap nn.Linear layers under `module` with LoRALinear."""
...
def enable_norm_affine(model: nn.Module, require_grad: bool = True):
    """Enable LayerNorm affine params (γ/β). Everything else remains frozen."""
...
def _get_vit_blocks(vit: nn.Module) -> List[nn.Module]:
    ...
    # Fallback: scan for modules that look like timm ViT blocks (have attn & mlp)
    candidates = []
...
def select_layers(num_blocks: int, policy: str) -> Dict[int, str]:
    """
    block_idx -> {"lora","norm","none"}
    Policies: even_lora | odd_lora | deep_lora
    """
    sel: Dict[int, str] = {}
    if policy == "even_lora":
...
# ---------------------------
def attach_lora_vit_block(block: nn.Module,
                          r: int = 8,
                          lora_alpha: float = 16.0,
                          lora_dropout: float = 0.0,
...
def build_method_vit(
    ...
    method:
      - "norm"             : NormTune only
      - "lora"             : LoRA only (all blocks)
      - "hybrid_parallel"  : LoRA + Norm on all blocks (parallel)
      - "hybrid_layerwise" : LoRA vs Norm per-block per 'policy'
      - "hybrid_layers"    : alias for hybrid_layerwise
    """
